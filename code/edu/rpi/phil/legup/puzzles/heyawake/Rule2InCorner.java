			...
			for (int y = 0; y < origBoardState.getHeight() && error == null; ++y)
			{
				for (int x = 0; x < origBoardState.getWidth(); ++x)
				{
					...
					if (origState != newState)
					{
						...
						if(curRegion.getValue()!=2)
						{
							error = "Rule cannot be applied to regions without a value of 2.";
						}
						dimPoint= getRegionDimension(origBoardState,regionNum);
						if(dimPoint.x!=1 && dimPoint.y!=1)
						{
							error = "Rule can only be applied to regions with dimension 2x2.";
							break;
						}
						locPoint = getRegionLocation(origBoardState,regionNum);
						if( !((locPoint.x==(width-2) || (locPoint.x==0)) && (locPoint.y==0 || locPoint.y==(height-2))) )
						{
							System.out.println(locPoint.x+" "+locPoint.y);
							error = "Rule can only be applied to regions against a corner.";
							break;
						}
					}
				}
			...
			for( int r = 0; r< regioncount; r++)
			{
				...
				if(curRegion.getValue() == 2)
				{
					...
					if(size.x == 1 && size.y == 1)
					{
						...
						if( ((location.x==0) && (location.y==0)) || ((location.x==(width-2)) && (location.y==(height-2))) )
						{
							changed=true;
						...
						else if( ((location.x==0) && (location.y==(height-2))) || ((location.x==(width-2)) && (location.y==0)) )
						{
							changed=true;
