...
{	 
    ...
    private boolean[][] determineBetween(BoardState state)
    {
    	...
    	for(int x = 0; x < width; ++x)
    	{
    		for(int y = 0; y < height; ++y)
    		{
    			...
    			{
    				if(loopConnected(state, x,y,width,height))
    				{
    ...
    private boolean loopConnected(BoardState boardState, int x, int y, int width, int height)
    {
    	...
    	if(x+1 < width)
    	{
    		if(loopConnected( boardState, x+1, y, width, height))
    			return true;
    	}
    	if(x-1 >= 0)
    	{
    	...
    	if(y+1 < height)
    	{
			if(loopConnected( boardState, x, y+1, width, height))
				return true;
    	}
    	if(y-1 >= 0)
    	{
